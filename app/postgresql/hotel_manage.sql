...
CREATE FUNCTION public.check_existed_id(id_acc text) RETURNS text
    LANGUAGE plpgsql
    AS $$
declare
	result text;
begin
    if (id_acc NOT IN (SELECT id_acc FROM tbl_account acc WHERE acc.id_acc = id_acc )) then
        result := id_acc;
    else
        result := '';
    end if;
...
ALTER FUNCTION public.check_existed_id(id_acc text) OWNER TO postgres;
...
1	admin	123	1	1	1	2021-01-10	2022-01-15
2	employee_001	1	2	2	1	2021-01-10	2022-01-15
\.
...
COPY public.tbl_check_in (id_ci, id_emp_ci, id_cus_ci, id_room_ci, numpeople_ci, datecheckin_ci, status_ci) FROM stdin;
\.
...
COPY public.tbl_customer (id_cus, name_cus, cardid_cus, birth_cus, phonenumber_cus, isforeigner_cus, status_cus) FROM stdin;
\.
...
SELECT pg_catalog.setval('public."tbl_Customer_ID_cus_seq"', 1, false);
...
SELECT pg_catalog.setval('public."tbl_check_in_ID_ci_seq"', 1, false);
